...
public class Area_AddmanagerCommand extends CommandExecutor {
    ...
    public Area_AddmanagerCommand(UUIDManager uuidManager, AreaManager areaManager, GroupManager groupManager, com.froobworld.froobpermissions.managers.PlayerManager fpPlayerManager, com.froobworld.froobbasics.managers.PlayerManager fbPlayerManager) {
    ...
    public boolean onCommandProcess(CommandSender sender, Command command, String cl, String[] args) {
        if (args.length < 3) {
            sender.sendMessage("/" + cl + " <area> addmanager <user>");
            return false;
        }
        ...
        if (area.getManagers().contains(user)) {
            sender.sendMessage(ChatColor.RED + "That user is already a manager of the area.");
            return false;
        }

        area.addManager(user);
        sender.sendMessage(ChatColor.YELLOW + "Added manager " + user.toString() + " to the area.");
        return true;
    }
    ...
    public String command() {
        ...
        return "area addmanager";
    }

    @Override
    public String perm() {

        return "froobprotect.area.addmanager";
    }
