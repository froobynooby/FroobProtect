...
public class Area_AddflagCommand extends CommandExecutor {
	...
	public Area_AddflagCommand(AreaManager areaManager, com.froobworld.froobpermissions.managers.PlayerManager fpPlayerManager, com.froobworld.froobbasics.managers.PlayerManager fbPlayerManager) {
	...
	public boolean onCommandProcess(CommandSender sender, Command command, String cl, String[] args) {
		if(args.length < 3) {
			sender.sendMessage("/" + cl + " <area> addflag <flag>");
			return false;
		}
		...
		if(sender instanceof Player && !sender.hasPermission(FroobProtect.PERM_EDIT_ALL_AREAS)) {
			...
			if(!area.getOwners().stream().anyMatch(u -> u.isUser(player, fpPlayerManager, fbPlayerManager))) {
				sender.sendMessage(ChatColor.RED + "You must be an owner of that area to add flags.");
				return false;
			}
		}
		...
		if(area.getFlags().contains(flag)) {
			sender.sendMessage(ChatColor.RED + "That area already has that flag.");
			return false;
		}
		
		area.addFlag(flag);
		sender.sendMessage(ChatColor.YELLOW + "Flag added.");
		return true;
	}
	...
	public String perm() {
		...
		return "froobprotect.area.addflag";
	}
	...
	public List<String> tabCompletions(CommandSender sender, Command command, String cl, String[] args) {
		...
		if(args.length == 3) {
			Area area = areaManager.getArea(args[0]);
			if(area != null) {
				for(Flag flag : areaManager.getFlags()) {
					if(area.getFlags().contains(flag) || !sender.hasPermission("froobprotect.flag." + flag.name())) {
						continue;
					}
